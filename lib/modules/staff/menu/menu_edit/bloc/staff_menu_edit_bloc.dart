...
class StaffMenuEditBloc extends Bloc<StaffMenuEditEvents, StaffMenuEditStates> {
  ...
  StaffMenuEditBloc(StaffMenuEditStates initialState, this._menuRepository)
      : super(initialState);

  @override
  Stream<StaffMenuEditStates> mapEventToState(
      StaffMenuEditEvents event) async* {
    ...
    if (event is ChangeEnabledStatus) {
      ...
      final DMTaskState result =
          await _menuRepository.setAvailableDay(event.id, event.days);
      if (result.isTaskSuccess) {
        yield StaffMenuEditSuccess();
      } else {
        yield StaffMenuEditError(result.error);
      ...
      final DMTaskState result =
          await _menuRepository.setAvailableTime(event.id, event.time);
      if (result.isTaskSuccess) {
        yield StaffMenuEditSuccess();
      } else {
        yield StaffMenuEditError(result.error);
